...
- **Smallest change that works**: Minimize blast radius; Feel free to refactor adjacent code, reuse, or tweek adjacent code if it meaningfully reduces risk, complexity, or boiler plate code.
- **Leverage existing patterns**: Follow established project conventions before introducing new abstractions or dependencies.
- **Prove it works**: "Seems right" is not done. Validate with tests/build and no errors or warnings in the Unity console.
...
All game assets live under `src/IncrementalAsteroidBoomerang/Assets`. Use these locations to minimize repository search time:
...
- **ScriptableObjects** → `src/IncrementalAsteroidBoomerang/Assets/ScriptableObjects`
- **2D Textures / sprites** → `src/IncrementalAsteroidBoomerang/Assets/Textures`
- **Shaders** → `src/IncrementalAsteroidBoomerang/Assets/Shaders`
- **Animations** → `src/IncrementalAsteroidBoomerang/Assets/Animations`
- **Fonts** → `src/IncrementalAsteroidBoomerang/Assets/Fonts`
- **Input actions** → `src/IncrementalAsteroidBoomerang/Assets/InputSystem_Actions.inputactions`
- **Runtime-loaded assets** → `src/IncrementalAsteroidBoomerang/Assets/Resources` (reserve for assets that must be loaded via `Resources.Load`)
- **Third-party / vendor** → `src/IncrementalAsteroidBoomerang/Assets/Plugins`, `/Assets/com.rlabrecque.steamworks.net`, `/Assets/TextMesh Pro`, `/Assets/Lana Studio` (do not modify unless explicitly needed)

When adding new assets, place them in the matching directory above. Do not create new top-level asset folders without justification.
...
- Never mark complete without evidence:
  - tests, typecheck, build, logs, or a deterministic manual repro.
...
1. **Plan First**
   ...
   - Include "Verify" tasks explicitly (tests/build/manual checks).
2. **Define Success**
...
- Always include:
  - what you ran (tests/build), and the outcome.
...
- If a change reveals deeper issues:
  ...
  - log follow-ups as TODOs/issues in github rather than expanding the current task.
...
- Keep changes reversible. Config gating or isolated commits.

### 5. Instrumentation as a Tool (Not a Crutch)
- In Unity MonoBehaviours, assume properties are setup unless we specifically plan for them to be optional. If they are not optional, instead of null checking and silently continuing, Debug.Log a warning that this gameObject needs to have the property reference set up. If you read this warning in the console modify the prefab with the correct reference.
- Otherwise add logging only when they materially reduce debugging time, or prevent recurrence.
...
- Tests/typecheck/build (as relevant) pass or you have a documented reason they were not run.
...
- [ ] Add/adjust tests
- [ ] Run verification (tests/build/manual repro)
